<?php
    ...
    if(isset($dados['btnCadastrar'])){
        $foto = $_FILES['imagem'];
        $dados['comparaSenhas'] = md5($dados['Confirmasenha']);
        $dados['senhas'] = md5($dados['senha']);
        $error = array();
        ...
        if (count($error) == 0) {
                ...
                if(move_uploaded_file($foto["tmp_name"], $caminho_imagem)){
                    ...
                    (nome,tipo,senha,usuario,genero,cpf,salario,cargaHoraria,dica,foto) VALUES
                    (:nome , :tipo, :senha ,:usuario, :genero , :cpf, :salario, :cargaHoraria, :dica,:foto)";
                    $cad_usuario = $conn->prepare($query_usuario);
                    $cad_usuario->bindParam(':nome', $dados['nome'], PDO::PARAM_STR);
                    $cad_usuario->bindParam(':tipo', $dados['permissao'], PDO::PARAM_STR);
                    $cad_usuario->bindParam(':senha',md5($dados['senha']), PDO::PARAM_STR);
                    ...
                    $cad_usuario->bindParam(':dica', $dados['dica'], PDO::PARAM_STR);
